...
export default {
  ...
  created() {
    // 在组件创建时打印 user 对象
    console.log('Received user object:', this.user);
  },
  methods: {
    submit() {
      console.log('userId:',this.user.userId)
      console.log('oldPassword:',this.userForm.oldPassword)
      this.$refs["form"].validate(valid => {

        if (valid) {

          if (this.user && this.user.userId) {
            updateUserPwd(this.user.userId, this.userForm.oldPassword, this.userForm.newPassword).then(response => {

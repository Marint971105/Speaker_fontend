                  ...
                  <p class="dimension-title">
                    ...
                    >
                      ...
                      <el-rate
                        ...
                        :max="5"
                        allow-half
                        @change="handleRatingChange('video', index, infoIndex)"
                      ></el-rate>
                    </div>
                </div>
              </div>
    ...
    async loadExistingEvaluation() {
      try {
        ...
        if (response.code === 1 && response.data) {
          // 查找自评数据
          const selfEvaluation = response.data.evaluationTypes.find(
            type => type.evaluationMethod === '自评'
          );
          
          if (selfEvaluation && selfEvaluation.evaluationContents) {
        ...
        console.error('加载已有自评数据失败:', error);

    ...
    <el-row class="header-row">
      <el-col :span="24">
        <div class="header">
          ...
          <span class="info-text">评价流程：师评</span>
...
<script>
import { showFile} from "@/api/myTask/myEvaluation/index";
  ...
  methods: {
    ...
    async fetchTaskData() {
      try {
        ...
        // 找到师评数据
        const teacherEvaluation = res.data.evaluationTypes.find(
          type => type.evaluationMethod === "师评"
        );

        if (!teacherEvaluation) {
          this.$message.error('未找到师评数据');
          return;
        }

        // 获取评分人（教师）姓名
        if (teacherEvaluation.reviewerId) {
          try {
            const reviewerInfo = await getInfoById(teacherEvaluation.reviewerId);
        ...
        this.submissionTypes = this.fixedTabOrder.filter(type =>
          teacherEvaluation.evaluationContents.some(
            content => content.evaluationContent === type
          )
        );

        // 处理每种类型的评价数据
        teacherEvaluation.evaluationContents.forEach(content => {
          const type = this.typeMapping[content.evaluationContent];

          if (type) {
            // 设置维度数据
            this.$set(this.evaluationData, type, content.evaluationDimensions.map(dim => {
              const dimensionKey = Object.keys(dim)[0];
              const dimensionValue = Number(dim[dimensionKey]);
              return {
                evaluationTitle: dimensionKey,
                evaluationInfos: [dimensionKey],
                score: dimensionValue
              };
            }));

            // 设置评分数据,将维度评分转换为ratings数组
            const dimensionRatings = content.evaluationDimensions.map(dim => {
              return [Number(Object.values(dim)[0])];
            });
            this.$set(this.ratings, type, dimensionRatings);

            // 设置评语
            this.$set(this.comments, type, content.evaluationTitle);
            // 设置总分
            console.log('Before update - Current dimensionScores:', {...this.dimensionScores}); // 使用解构来获取当前状态的快照
            console.log('Setting grade for type:', content.evaluationContent, 'grade:', content.grade);

            this.$set(this.dimensionScores, content.evaluationContent, content.grade);
            this.$nextTick(() => {
              console.log('After update - dimensionScores:', {...this.dimensionScores});
            });
    ...
    calculateTotalScore(type) {
      ...
      // 使用 grade 作为分数
      const score = this.dimensionScores[chineseType] || 0;
      // 不需要再次更新 dimensionScores，因为这个值已经在 fetchTaskData 中设置过了
      // 只需要更新雷达图
